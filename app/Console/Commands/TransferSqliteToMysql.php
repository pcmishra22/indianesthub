...
class TransferSqliteToMysql extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($tables as $tableRow) {
            ...
            // Aim for ~60,000 placeholders max per query for safety
            $chunkSize = $columnCount > 0 ? (int) floor(60000 / $columnCount) : 1000;
            $chunkSize = max(1, min($chunkSize, 1000)); // Ensure at least 1, cap at 1000
            ...
            $sqliteConn->table($table)->orderByRaw('1')->chunk($chunkSize, function ($rows) use ($mysqlConn, $table) {
                ...
                // Handle empty tables/chunks gracefully
                if (!empty($data)) {
                    // We use insert instead of create to bypass model events/mass-assignment
