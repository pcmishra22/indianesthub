...
async function loadEngineRollup(engine){
  ...
  try{
    ...
    const pendingNote=pending>0?` · <span style="color:var(--orange)">${pending} still open</span>`:'';
    el.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
      <div style="font-size:15px;color:var(--muted2)">
        ...
        <strong style="color:#fff">${today.achieved}/${today.total}</strong> targets hit${pendingNote} ·
        <strong style="color:${rateColor}">${rate}</strong> <span style="font-weight:400">of decided</span>
      </div>
    ...
    <div style="font-size:12px;color:var(--muted);margin-top:6px">Counts only picks that got an official entry price + target (the boxes and "Current Top Gainer/Loser" cards above) — the Leaderboard further up shows more candidates by score, but not all of them turn into a locked-in, tracked trade. Rate is of picks that have hit target or closed unresolved — still-open picks aren't counted against it yet.</div>`;
  }catch(e){
