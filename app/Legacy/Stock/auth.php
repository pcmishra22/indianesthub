...
function stockRedirect(string $path): void {
    global $_base;
...
<div class="card">
  ...
  <p class="hint" style="margin-top:8px"><a href="register" style="color:#60a5fa;text-decoration:none">Create a new account</a></p>
</div>
</body>
</html>
<?php }

function registerPage(string $appName, string $err): void { ?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
...
.btn{width:100%;padding:13px;background:linear-gradient(135deg,#0072ff,#00c6ff);border:none;border-radius:10px;color:#fff;font-size:1rem;font-weight:600;cursor:pointer}
.err{background:rgba(239,68,68,.15);border:1px solid rgba(239,68,68,.3);color:#f87171;padding:12px 16px;border-radius:10px;margin-bottom:20px;font-size:.875rem}
.ok{background:rgba(16,185,129,.15);border:1px solid rgba(16,185,129,.25);color:#34d399;padding:12px 16px;border-radius:10px;margin-bottom:20px;font-size:.875rem}
</style>
</head>
<body>
<div class="card">
  ...
  <p class="sub">Set up a personal watchlist and recommendation history</p>
  <?php if ($err): ?><div class="err">⚠ <?= htmlspecialchars($err) ?></div><?php endif; ?>
  <form method="POST">
    <label>Username</label><input type="text" name="u" required autocomplete="username">
    <label>Password</label><input type="password" name="p" required autocomplete="new-password">
    <button class="btn">Create account →</button>
  </form>
  <p style="margin-top:16px;text-align:center;font-size:.8rem;color:#6b7280"><a href="login" style="color:#60a5fa;text-decoration:none">Back to login</a></p>
</div>
</body>
</html>
<?php }
...
if ($uri === '/register' || $uri === '/register/') {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        registerPage($APP_NAME, 'That username already exists.'); exit;
    }
    registerPage($APP_NAME, ''); exit;
}
...
if ($uri === '/logout' || $uri === '/logout/') {
    session_destroy(); stockRedirect('login');
}

if (empty($_SESSION['auth'])) { stockRedirect('login'); }
