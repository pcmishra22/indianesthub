...
function prakashRollupHistory(?string $username = null, int $maxDays = 90, bool $withDetails = false): array
{
    ...
    foreach ($files as $file) {
        ...
        // A pick only has a *known outcome* once it either hits its target
        // (achieved) or the day has closed and it's finalized as Not
        // Achieved (see closePrakashDailyIfNeeded()). Anything still open
        // on a day that isn't closed yet hasn't resolved either way — it
        // must not be counted as a "miss" just because it hasn't hit yet.
        // Previously every not-yet-achieved pick (including ones opened
        // minutes ago) counted against the rate, so "today's win rate"
        // fell toward 0% early in the session regardless of actual
        // performance and only recovered as picks resolved.
        $achieved = 0; $failed = 0; $pending = 0;
        foreach ($recs as $r) {
            if (!is_array($r)) continue;
            if (!empty($r['achieved'])) { $achieved++; }
            elseif ($closed) { $failed++; }
            else { $pending++; }
        }
        $count = count($recs);
        $resolved = $achieved + $failed;
        ...
        $dayEntry = [
            ...
            'failed' => $failed,
            'pending' => $pending,
            // Rate is now "of the picks that have actually resolved, how
            // many hit" — not diluted by ones still in flight. Null (not
            // 0%) when nothing has resolved yet, so the UI can show "—"
            // instead of a false 0%.
            'success_rate' => $resolved > 0 ? round($achieved / $resolved * 100, 1) : null,
            'closed' => $closed,
        ];
    ...
    $totalResolved = $totalAchieved + $totalFailed;
    return [
        'days' => $days,
        'overall_total' => $totalCount,
        'overall_achieved' => $totalAchieved,
        'overall_failed' => $totalFailed,
        'overall_pending' => $totalPending,
        'overall_resolved' => $totalResolved,
        'overall_success_rate' => $totalResolved > 0 ? round($totalAchieved / $totalResolved * 100, 1) : null,
    ];
}
...
function buildPrakashRecommendations(array $stocks, ?string $statePath = null, ?string $historyPath = null, ?string $username = null, ?string $rankHistoryPath = null, ?string $topSeenPath = null): array
{
    ...
    // ── Queue ordering: FIFO, not "newest bumps oldest" ────────────────
    // Previously sorted newest-entry-time-first and sliced to $boxCap —
    // so the moment a 6th open pick registered, the OLDEST still-open
    // one (e.g. a Titan buy given first thing this morning) silently
    // dropped off the visible box, replaced by a brand-new stock with a
    // different price/target. The underlying locked entry_price/
    // target_price in $daily['recommendations'] was never actually
    // touched — it's still there, correct, in the EOD report/rollup —
    // but from the box alone this reads exactly like "my recommendation
    // got overwritten". Sorting oldest-first instead means a pick that
    // takes a box slot KEEPS it until it actually resolves (target hit,
    // or the day closes) — new picks queue up behind it and only
    // surface once a slot genuinely frees up. That's the FIFO queue
    // behaviour the box is meant to have.
    usort($openBuyBox, fn($a, $b) => strcmp($a['_entry_time'], $b['_entry_time']));
    usort($openSellBox, fn($a, $b) => strcmp($a['_entry_time'], $b['_entry_time']));
