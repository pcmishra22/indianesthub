...
function aiRollupHistory(?string $username = null, int $maxDays = 90, bool $withDetails = false): array
{
    ...
    foreach ($files as $file) {
        ...
        // See prakash_recommendations.php's prakashRollupHistory() for why:
        // a pick still open on a day that hasn't closed yet has no known
        // outcome and must not count against the success rate.
...
function buildAiRecommendations(array $stocks, ?string $statePath = null, ?string $historyPath = null, ?string $username = null): array
{
    ...
    // Oldest open pick keeps its box slot until it resolves — see
    // prakash_recommendations.php's matching fix for why (newest-first
    // eviction was silently dropping still-open older picks from view).
