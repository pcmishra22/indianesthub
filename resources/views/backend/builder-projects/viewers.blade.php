...
@section('title', 'Project Viewers')

@section('content')
<div class="d-flex align-items-center justify-content-between mb-3">
    <h4 class="mb-0">👁️ Viewers for: {{ $project->title }}</h4>
    <a href="{{ route('admin.builder-projects.show', $project->id) }}" class="btn btn-sm btn-outline-secondary">Back</a>
</div>
    ...
    <div class="col-12 col-md-4">
        <div class="card shadow-sm">
            <div class="card-body">
                ...
                <div class="fs-4 fw-bold text-primary">{{ number_format($projectViews->total()) }}</div>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered table-sm align-middle mb-0">
                ...
                @empty
                    <tr><td colspan="8" class="text-center text-muted">No viewers yet for this project.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
        @if($projectViews->hasPages())
        <div class="mt-3">{{ $projectViews->links() }}</div>
        @endif
    </div>
</div>
@endsection
