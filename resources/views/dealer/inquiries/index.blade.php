...
<div class="container-fluid p-0">
  ...
  <div class="mb-4 d-flex align-items-center justify-content-between flex-wrap gap-2">
    <div>
      ...
      <p class="text-muted mb-0">Track, score, and follow up on every lead from your properties</p>
    </div>
    <a href="{{ route('dealer.inquiries.export', request()->query()) }}"
  ...
  <div class="bl-filters">
    <form action="{{ route('dealer.inquiries.index') }}" method="GET" class="row g-2 align-items-end">
      <div class="col-md-3">
      ...
      <div class="col-md-2">
        ...
        <select name="property_id" class="form-select form-select-sm">
          ...
          <option value="{{ $prop->id }}" {{ request('property_id') == $prop->id ? 'selected' : '' }}>{{ Str::limit($prop->title, 28) }}</option>
          @endforeach
        </select>
      </div>
      <div class="col-md-2">
        ...
        <select name="lead_type" class="form-select form-select-sm">
          ...
          <option value="facebook_lead" {{ request('lead_type') === 'facebook_lead' ? 'selected' : '' }}>Facebook/Instagram</option>
        </select>
      </div>
      <div class="col-md-2">
      ...
      <div class="col-md-1 d-flex gap-1">
        ...
        @if(request()->anyFilled(['search','status','lead_type','property_id','heat']))
        <a href="{{ route('dealer.inquiries.index') }}" class="btn btn-outline-secondary btn-sm px-2"><i class="bi bi-x"></i></a>
        @endif
      </div>
    </form>
  </div>
  ...
  <div class="bl-table-wrap">
        ...
        <tbody>
          ...
          @php
            ...
            $typeIcons = [
              ...
              'facebook_lead' => ['bi-facebook',          '#1877f2', 'Facebook/Insta'],
            ];
            $ti = $typeIcons[$inq->lead_type] ?? ['bi-question-circle', '#94a3b8', ucfirst($inq->lead_type)];
            $callCount = count($inq->call_log ?? []);
          @endphp
          <tr>
            <td>
              <span class="heat-badge {{ $heatConfig[1] }}" title="Score: {{ $inq->hot_score }}">
                {{ $heatConfig[0] }} {{ $heatConfig[2] }}
              </span>
              <div style="font-size:.65rem;color:#94a3b8;margin-top:2px;text-align:center;">{{ $inq->hot_score }}/100</div>
            </td>

            <td>
              <div style="font-weight:700;color:#1e293b;">{{ $inq->name }}</div>
              @if($inq->message)
              <div style="font-size:.74rem;color:#94a3b8;max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="{{ $inq->message }}">
                {{ $inq->message }}
              </div>
              @endif
              @if($callCount)
            ...
            <td>
              <a href="tel:{{ $inq->phone }}" class="d-block text-decoration-none fw-500" style="color:#1e293b;font-size:.83rem;">
                <i class="bi bi-telephone me-1 text-success"></i>{{ $inq->phone ?: '—' }}
              </a>
              @if($inq->email)
              <a href="mailto:{{ $inq->email }}" class="d-block text-decoration-none" style="color:#64748b;font-size:.75rem;margin-top:2px;">
                <i class="bi bi-envelope me-1"></i>{{ $inq->email }}
              </a>
              @endif
            </td>
            ...
            <td>
              @if($inq->property)
              <span style="font-size:.8rem;color:#6366f1;font-weight:500;">{{ Str::limit($inq->property->title, 28) }}</span>
              @else
            ...
            <td>
              @if($inq->follow_up_at)
                @if($inq->follow_up_at->isPast())
                  <span class="overdue-chip"><i class="bi bi-exclamation-triangle me-1"></i>Overdue</span>
                @elseif($inq->follow_up_at->isToday())
                  <span class="upcoming-chip"><i class="bi bi-bell me-1"></i>Today</span>
                @else
                  <span class="done-chip"><i class="bi bi-calendar me-1"></i>{{ $inq->follow_up_at->format('d M') }}</span>
                @endif
              @else
            ...
            <td>
              <select class="form-select form-select-sm status-changer" data-lead-id="{{ $inq->id }}"
                ...
                <option value="Lost"      {{ $inq->status === 'Lost'      ? 'selected' : '' }}>⚫ Lost</option>
              </select>
            </td>

            <td style="white-space:nowrap;">
              <span style="font-size:.78rem;color:#64748b;">{{ $inq->created_at->format('d M Y') }}</span><br>
              <span style="font-size:.7rem;color:#94a3b8;">{{ $inq->created_at->diffForHumans() }}</span>
            </td>
            ...
            <td style="text-align:right;padding-right:14px;white-space:nowrap;">
              <button type="button" class="bl-action-btn open-drawer-btn"
                      ...
                      data-property="{{ $inq->property->title ?? 'General' }}"
                      data-type="{{ $inq->lead_type }}"
                      data-status="{{ $inq->status }}"
                      data-score="{{ $inq->hot_score }}"
                      data-heat="{{ $heat }}"
                      data-message="{{ $inq->message ?? '' }}"
                      data-notes="{{ $inq->notes ?? '' }}"
                      data-followup="{{ $inq->follow_up_at?->format('Y-m-d\TH:i') ?? '' }}"
                      data-calllog='@json($inq->call_log ?? [])'
                      data-received="{{ $inq->created_at->format('d M Y, h:i A') }}"
                      data-notes-url="{{ route('dealer.inquiries.save-notes', $inq) }}"
                      data-calllog-url="{{ route('dealer.inquiries.add-call-log', $inq) }}"
                      title="View Details">
                <i class="bi bi-eye"></i>
              </button>
              <a href="tel:{{ $inq->phone }}" class="bl-action-btn success" title="Call {{ $inq->name }}">
                <i class="bi bi-telephone-fill"></i>
              </a>
              <a href="https://wa.me/91{{ preg_replace('/[^0-9]/', '', $inq->phone) }}"
                 target="_blank" class="bl-action-btn" style="color:#25d366;border-color:#86efac;" title="WhatsApp">
                <i class="bi bi-whatsapp"></i>
              </a>
              <form action="{{ route('dealer.inquiries.destroy', $inq) }}" method="POST" class="d-inline"
    ...
    <div class="text-center py-5">
      ...
      <p class="text-muted" style="font-size:.88rem;">
        @if(request()->anyFilled(['search','status','lead_type','property_id','heat']))
          No leads match your filters. <a href="{{ route('dealer.inquiries.index') }}">Clear filters</a>
        @else
...
<div id="lead-drawer">
  ...
  <div class="ld-body">
    ...
    <div class="row g-3 mb-4">
      <div class="col-6">
        <div class="ld-label">Property</div>
        <div class="ld-value" id="ld-property">—</div>
      </div>
      <div class="col-6">
...
document.querySelectorAll('.status-changer').forEach(function(sel) {
  sel.addEventListener('change', function() {
    ...
    fetch(`/dealer/inquiries/${leadId}/status`, {
      method: 'PATCH',
...
function openDrawer(btn) {
  ...
  document.getElementById('ld-property').textContent = btn.dataset.property;
  ...
  document.getElementById('ld-status-display').textContent = btn.dataset.status;
